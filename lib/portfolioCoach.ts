 ...
 *
 * This satisfies:
 * - Shipyard (Josh's brief): Portfolio tracking with smart insights
 * - Comet Opik: AI agent helping users stick to financial goals
 * - Gemini: Powered by Gemini API
 */
...
import { calculateMetrics, identifyConcentrationRisks } from './portfolioAnalysis';
...
export interface PortfolioInsight {
  id: string;
  type: 'goal_progress' | 'rebalance_needed' | 'concentration_warning' | 'milestone' | 'strategy_drift' | 'action_reminder' | 'market_context';
 ...
 */
export async function recordPortfolioCheckIn(): Promise<{ streak: number; isNewDay: boolean }> {
  const state = await getCoachState();
  const today = new Date().toDateString();
  const lastCheckIn = state.lastCoachingDate ? new Date(state.lastCoachingDate).toDateString() : null;
...
): { assetClass: AssetClass; current: number; target: number; deviation: number }[] {
  const deviations: { assetClass: AssetClass; current: number; target: number; deviation: number }[] = [];

  const allClasses: AssetClass[] = ['equity', 'debt', 'commodity', 'real_asset', 'cash'];
...
export async function generatePortfolioGreeting(
  ...
  const metrics = context.holdings.length > 0 ? calculateMetrics(context.holdings) : null;
  ...
  if (metrics && metrics.topHoldings[0]?.percent > 30) conditions.push('concentrated position');
...
export async function generatePortfolioInsights(
  ...
  if (context.holdings.length === 0) {
    insights.push({
      ...
      message: 'Add your first investment to begin tracking your portfolio and get personalized insights.',
  ...
  if (context.goals?.targetValue && context.totalValue > 0) {
    const progressPercent = (context.totalValue / context.goals.targetValue) * 100;
  ...
  if (context.goals?.targetAllocation) {
    const deviations = calculateAllocationDeviation(currentAllocation, context.goals.targetAllocation);
    const significantDeviations = deviations.filter((d) => Math.abs(d.deviation) > 10);
...
async function generateAIInsight(
  ...
  const schema = z.object({
    type: z.enum(['strategy_drift', 'action_reminder', 'market_context', 'goal_progress']),
  ...
  const holdingsSummary = context.holdings.slice(0, 10).map((h) => ({
    ...
    gainPercent: h.purchasePrice > 0
      ? (((h.currentPrice || h.purchasePrice) - h.purchasePrice) / h.purchasePrice) * 100
      : 0,
  }));
...
Return JSON with: type, priority, title (under 25 chars), message (under 100 chars), actionLabel (optional, under 15 chars)`;
  ...
  try {
    ...
    return {
      ...
      ...result,
      actionRoute: result.type === 'action_reminder' ? '/portfolio/add' : '/portfolio/analysis',
      createdAt: new Date().toISOString(),
    };
  } catch (error) {
 ...
 */
export async function generateDailyTip(context: PortfolioCoachingContext): Promise<string> {
  const traceId = await opik.createTrace({
    name: 'portfolio_daily_tip',
    input: { strategy: context.goals?.strategy, riskTolerance: context.goals?.riskTolerance },
    tags: ['coaching', 'tip'],
  });
  ...
  const randomTip = strategyTips[Math.floor(Math.random() * strategyTips.length)];
